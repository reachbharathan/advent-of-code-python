...
def final_frequency_result_list(initial_list,input):
    result_list = initial_list
    result = initial_list[len(initial_list)-1]
    for x in input:
        result += x
        result_list.append(result)
    return result_list

def find_repeat_frequency_by_iteration(input):
    input_result_frq = {}
    result = {}
    for x in input:
        if input_result_frq.has_key(x):
            input_result_frq[x] += 1
            result[True]=x
            return result
        else:
            input_result_frq[x] = 1
...
def find_repeat_frequency(input_list):
    input_result_frequency = {}
    result_list = final_frequency_result_list([0],input_list)
    repeat = True
    count = 1
    while repeat:
        result_di = find_repeat_frequency_by_iteration(result_list)
        if result_dic.has_key(True):
            return result_dic.get(True)
        else:
            print("Iteration %d"%count)
            result_list = final_frequency_result_list(result_list,input_list)
            count+=1
...
print(find_repeat_frequency(input))
